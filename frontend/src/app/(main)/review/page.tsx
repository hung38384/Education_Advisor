                    ...
                    <>
                        ...
                        <div className="grid gap-3 xl:grid-cols-2">
                            {result.recommendations.slice(0, 15).map((item) => {
                                const universityName = formatUniversityName(item.universityName, item.universityCode);
                                const method = item.featuredMethod;
                                const methodName = method ? formatAdmissionMethodName(method.methodAlias, method.methodTag) : null;

                                return (
                                    <div
                                        key={`${item.universityCode}:${item.majorCode}`}
                                        className="flex h-full flex-col gap-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm transition hover:border-slate-300 hover:shadow-md"
                                    >
                                        ...
                                        {method && (
                                            <div className="rounded-md bg-slate-50 p-3 text-sm text-slate-700">
                                                <p className="font-medium text-slate-900">
                                                    Phương thức nổi bật: {methodName}
                                                </p>
